...
//   const { momByMeetingId, momByMeetingIdLoading, momView, momViewLoading, error } = useSelector(
//     (state) => state.mom
//   );
...
//   const prevMeetingIdRef = useRef(null); // Track previous meetingId to prevent redundant API calls
...
//       start: { dateTime: meeting?.start?.dateTime || "" },
//       end: { dateTime: meeting?.end?.dateTime || "" },
...
//     projectName: project?.projectName || "", // Initialize projectName
//     status: "draft",
//   });
...
//     const start = formatTimes(adaptedMeeting.start.dateTime);
//     const end = formatTimes(adaptedMeeting.end.dateTime);
...
//     if (!adaptedMeeting.end.dateTime) return { isTimeExceeded: false, isWithinOneHour: false };
//     const endTime = new Date(adaptedMeeting.end.dateTime);
//     const now = new Date();
...
//           : momByMeetingId.participants || getAttendeesArray(adaptedMeeting.attendees).join(", ") || "",
//         summary: momByMeetingId.summary || "",
//         notes: momByMeetingId.notes || "",
//         createdBy: momByMeetingId.createdBy || currentUser?.name || "",
//         meetingId: adaptedMeeting.meetingId || "",
//         projectName: momByMeetingId.projectName || project?.projectName || "", // Set projectName from momByMeetingId or project
...
//         participants: getAttendeesArray(adaptedMeeting.attendees).join(", ") || "",
//         summary: "",
//         notes: "",
//         createdBy: currentUser?.name || "",
//         meetingId: adaptedMeeting.meetingId || "",
//         projectName: project?.projectName || "", // Set projectName for new MoM
//         status: "draft",
//       });
//       setMode("form");
//       setIsEditMode(false);
...
//   useEffect(() => {
//     if (momByMeetingId?.momId && mode === "view" && !momViewLoading) {
//       dispatch(fetchMoMView(momByMeetingId.momId));
//     }
//   }, [momByMeetingId?.momId, mode, dispatch, momViewLoading]);
...
//   // Handler: Form input changes (non-debounced for immediate response)
...
//       formData.append("projectName", momForm.projectName); // Add projectName to formData
...
//         {/* Loading State */}
//         {momByMeetingIdLoading || momViewLoading ? (
//           <div className="min-h-[400px] bg-background rounded-xl flex items-center justify-center">
...
//         toast.success(`MoM ${status === "draft" ? "updated as draft" : "finalized"} successfully!`);
//       } else {
//         // Create new MoM and submit cause if necessary
//         await dispatch(createProjectMoM(formData)).unwrap();
//         toast.success(`MoM ${status === "draft" ? "saved as draft" : "created"} successfully!`);

//         // Submit cause only if createProjectMoM is successful and conditions are met
//         if (status === "final" && isTimeExceeded && !isEditMode) {
//           await dispatch(
//             submitCause({
//               meetingId: adaptedMeeting.meetingId,
...
//             {/* Header */}
//             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
//               <h2 className="text-xl sm:2xl font-bold text-primary flex items-center">
//                 <FileText className="h-5 w-5 sm:h-6 sm:w-6 mr-2" />
//                 {mode === "form" ? (isEditMode ? "Edit Minutes of Meeting" : "Create Minutes of Meeting") : "Meeting Details"}
//               </h2>
//               {mode === "view" && momByMeetingId && (
  ...
  const handleSubmit = async (status) => {
    ...
    try {
      ...
      if (isEditMode && momByMeetingId) {
        ...
        createMoMSuccess = !!createMoMResult; // Check if createMoMResult indicates success
        toast.success(`MoM ${status === "draft" ? "saved as draft" : "created"} successfully!`);
      }

      // Submit cause only if createProjectMoM was successful and conditions are met
